...
void glazunova_7_2_basic() {
    string text = "Это строка с точками, запятыми, и другими символами. Вот еще: один.";
    long dots_count = 0;
    long commas_count = 0;

    // --- Метод 1: Использование std::count ---
    dots_count = ranges::count(text, '.');
    commas_count = ranges::count(text, ',');

    /*
    // --- Метод 2: Ручной подсчет в цикле ---
    for (char c : text) {
        if (c == '.') {
            dots_count++;
        } else if (c == ',') {
    ...
    */
    ...
    cout << "--- Задание 7.2 (Базовый) ---" << endl;
    cout << "Исходная строка: \"" << text << "\"" << endl;
    cout << "Количество точек: " << dots_count << endl;
    cout << "Количество запятых: " << commas_count << endl;
}
...
void glazunova_7_2_middle() {
    string text = "Это строка со скобками, (которые нужно) удалить.";
    char char_to_remove = '(';

    // Находим позицию первого вхождения символа
    size_t pos = text.find(char_to_remove);

    cout << "--- Задание 7.2 (Средний) ---" << endl;
    cout << "Исходная строка: \"" << text << "\"" << endl;
    cout << "Символ для удаления: '" << char_to_remove << "'" << endl;
    ...
    if (pos != string::npos) {
        ...
        cout << "Результат: Символ '" << char_to_remove << "' не найден. Строка не изменена." << endl;
    }
}
...
void glazunova_7_2_high() {
    string text = "Это* тестовая* строка* с несколькими* звездочками.";
    int counter = 0; // Счетчик вхождений символа '*'
    char target_char = '*';

    cout << "--- Задание 7.2 (Высокий) ---" << endl;
    cout << "Исходная строка: \"" << text << "\"" << endl;
    ...
    for (size_t i = 0; i < text.length(); ++i) {
        if (text[i] == target_char) {
            ...
            string replacement = to_string(counter);
            ...
            // (длина вставленной строки - 1, так как 1 символ мы уже удалили)
