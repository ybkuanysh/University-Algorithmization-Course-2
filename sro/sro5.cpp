...
// Структура для входного файла и списка LIn
struct Rec1 {
    long nomer;
    char name[20];
    int year;
...
// Структура для выходного списка LOut и файла FOut
struct Rec2 {
    char name[20];
    int year;
    char group[20];
...
Rec1 readRec1FromConsole(long index) {
    ...
    cin.clear();
    // Используем cin.getline, чтобы избежать проблем с пробелами
    ...
    // После ввода числа, буфер может остаться грязным, если потребуется следующий ввод
    // clearInputBuffer();
...
long getNextNomer(const char *fileName) {
    ...
    // Предполагаем, что файл всегда содержит только Rec1
...
/**
 * Функция выполняет обработку данных по схеме FIn -> LIn -> LOut -> FOut.
 * Вариант 11: Найти самого молодого студента (максимальный год рождения)
 * с максимальным средним баллом среди них.
 */
void CreateFileOut(const char *fileName1, const char *fileName2) {
    // ----------------------------------------------------------------------
    // FIn -> LIn (Загрузка всего содержимого входного файла в список)
    // ----------------------------------------------------------------------
    ...
    vector<Rec1> LIn; // Линейный список LIn
    Rec1 rec;
    ...
    if (LIn.empty()) {
        cout << "Входной список LIn пуст. Выходной файл не создан." << endl;
        return;
    }

    // ----------------------------------------------------------------------
    // LIn -> LOut (Обработка списка LIn и создание результирующего списка LOut)
    // ----------------------------------------------------------------------
    cout << "Обработка данных: LIn -> LOut..." << endl;

    // 2.1. Находим максимальный год (самый молодой студент) только в списке LIn
    int max_year = LIn[0].year;
    for (const auto& student : LIn) {
        if (student.year > max_year) {
            max_year = student.year;
        }
    }

    // 2.2. Находим максимальный средний балл среди студентов с максимальным годом
    float max_sred = -1.0f;
    for (const auto& student : LIn) {
        if (student.year == max_year) {
    ...
    vector<Rec2> LOut; // Результирующий линейный список LOut

    // 2.3. Формируем LOut, отбирая подходящие записи из LIn
    for (const auto& student : LIn) {
        if (student.year == max_year && student.sred == max_sred) {
            Rec2 recOut;
            // Копируем данные из Rec1 (LIn) в Rec2 (LOut)
            strcpy(recOut.name, student.name);
            recOut.year = student.year;
            strcpy(recOut.group, student.group);
            recOut.sred = student.sred;

            LOut.push_back(recOut);
        }
    }

    // ----------------------------------------------------------------------
    // 3. ЭТАП: LOut -> FOut (Перемещение информации из списка LOut в выходной файл)
    // ----------------------------------------------------------------------
    fstream fileOut(fileName2, ios::out | ios::binary | ios::trunc); // Создаем/перезаписываем FOut
    if (!fileOut) {
        cerr << "Ошибка: Не удалось открыть выходной файл " << fileName2 << " для записи." << endl;
        return;
    }

    // Записываем все элементы списка LOut в FOut
    for (const auto& recOut : LOut) {
        fileOut.write(reinterpret_cast<const char*>(&recOut), sizeof(Rec2));
    }

    fileOut.close();

    cout << "Обработка завершена. Найдено " << LOut.size()
         << " студентов (самый молодой год: " << max_year
         << ", максимальный балл среди них: "
         << fixed << setprecision(2) << max_sred << ")." << endl;
    cout << "Результаты записаны из списка LOut в файл " << fileName2 << "." << endl;
}
...
void menu() {
    ...
    do {
        ...
        cout << "4. CreateFileOut: Обработать (FIn->LIn->LOut->FOut) и создать выходной файл (" << OUT_FILE << ")" << endl;
        ...
        switch (choice) {
            ...
            case 3:
                ...
                break;
            case 4:
                // Здесь вызывается измененная функция, реализующая схему FIn -> LIn -> LOut -> FOut
...
int main() {
    // Включаем поддержку русского языка в консоли, если это возможно
